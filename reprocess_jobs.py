...
from src.config.companies import get_all_companies, get_sector_for_company
...
def get_sector_by_company_name(company_name: str) -> str | None:
...
def reprocess_job(job_data: dict, file_path: Path) -> dict:
    ...
    if titulo_atual == "Outro" and novo_titulo != "Outro":
        logger.info(
            "Titulo corrigido", original=titulo_original[:50], de=titulo_atual, para=novo_titulo
        )
    ...
    if not localidade_atual:
        ...
        if loc_info.get("cidade") or loc_info.get("estado"):
            ...
            logger.debug(
                "Localidade extraida", original=texto_busca[:50], localidade=nova_localidade
            )
        ...
        if isinstance(localidade_atual, str):
            ...
            if nova_localidade != localidade_atual:
                logger.debug("Localidade normalizada", de=localidade_atual, para=nova_localidade)
...
def analyze_before_after(jobs: list[dict]) -> dict:
    ...
    for job in jobs:
        ...
        if (
            vaga_antes.get("titulo_normalizado") == "Outro"
            and vaga_depois.get("titulo_normalizado") != "Outro"
        ):
...
def main():
    ...
    print("TITULOS:")
    print(
        f"  'Outro' antes:  {stats['titulos_outro_antes']} ({100 * stats['titulos_outro_antes'] / stats['total']:.1f}%)"
    )
    print(
        f"  'Outro' depois: {stats['titulos_outro_depois']} ({100 * stats['titulos_outro_depois'] / stats['total']:.1f}%)"
    )
