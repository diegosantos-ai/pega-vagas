    ...
    def filter_jobs(self, jobs: list[dict]) -> list[dict]:
        ...
        for job in jobs:
            ...
            if normalized_title == "Outro":
                ...
                if not any(
                    kw in title_lower
                    for kw in ["data", "dados", "full stack", "backend", "back end"]
                ):
                    stats["wrong_title"] += 1
                    continue
        ...
        logger.info("Filtragem concluída", total=len(jobs), validas=len(valid), **stats)
        return valid
...
async def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Preview sem enviar para Telegram")
    parser.add_argument(
        "--platforms-only", action="store_true", help="Buscar apenas em plataformas"
    )
    parser.add_argument(
        "--companies-only", action="store_true", help="Buscar apenas em empresas foco"
    )
