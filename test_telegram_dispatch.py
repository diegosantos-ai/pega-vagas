...
from src.processing.validators import is_valid_job, filter_jobs
    ...
    for json_file in silver_dir.glob("*_processed.json"):
        try:
            ...
            all_jobs.append({
                ...
                "salario_min": vaga.get("salario", {}).get("valor_minimo") if vaga.get("salario") else None,
                "salario_max": vaga.get("salario", {}).get("valor_maximo") if vaga.get("salario") else None,
...
async def send_jobs(jobs: list[dict], dry_run: bool = False):
    ...
    if dry_run:
        ...
        for i, job in enumerate(jobs, 1):
            ...
            print(f"   Skills: {', '.join(notification.skills[:5]) if notification.skills else 'N/A'}")
            print(f"   URL: {notification.url[:60]}..." if len(notification.url) > 60 else f"   URL: {notification.url}")
            print()
        return
    ...
    await notifier.send_message(
        ...
        f"Enviando {total} vaga(s) do banco de dados...",
        parse_mode="Markdown"
    )
...
def main():
    ...
    parser.add_argument("--no-filter", action="store_true", help="Desabilita filtros de remoto/Brasil")
